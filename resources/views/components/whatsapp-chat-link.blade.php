...
@php
  $whatsappUrl =
      ...
      urlencode(
          "Hi, saya tertarik dengan {$product->name} (Rp " .
...
href="{{ $whatsappUrl }}"
  ...
  @else
  href="{{ route('login') }}"
  @click.prevent="$dispatch('toast', { message: @js('Silakan login terlebih dahulu untuk chat via WhatsApp.'), duration: 3000, actionText: 'Login', actionUrl: @js(route('login')) })" @endauth
