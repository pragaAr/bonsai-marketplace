...
@auth
  ...
  >
    ...
    @if($label)
      <span class="{{ $spanClass }}" x-text="loading ? 'Menambahkan…' : '{{ $label }}'"></span>
    @endif
  </button>
@else
  <a
    href="{{ route('login') }}"
    @click.prevent="$dispatch('toast', { message: @js('Silakan login terlebih dahulu untuk menambahkan produk ke keranjang.'), duration: 3000, actionText: 'Login', actionUrl: @js(route('login')) })"
    aria-label="Add {{ $product->name }} to cart"
    {{ $attributes }}
  >
    <!-- Plus icon -->
