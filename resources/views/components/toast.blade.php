<div x-data="{
  ...
  <template x-for="toast in toasts" :key="toast.id">
    <div
      ...
      <svg class="w-4 h-4 text-accent flex-shrink-0"
      ...
      <a x-show="toast.actionUrl" :href="toast.actionUrl"
        class="flex-shrink-0 rounded-full bg-cream px-3 py-1 text-xs font-semibold text-primary transition-colors hover:bg-accent">
        <span x-text="toast.actionText ?? 'Buka'"></span>
      </a>
      <button @click="_remove(toast.id)"
        class="text-cream/70 hover:text-cream flex-shrink-0 transition-colors p-1 rounded-full hover:bg-white/10"
