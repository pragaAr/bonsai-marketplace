  ...
  <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-24">
    ...
    <div
          ...
          <div
            ...
            <div>
              ...
              <p class="font-medium text-primary mt-0.5">
                {{ $article->author ? $article->author->name : 'Komunitas Bonsaiku' }}
              </p>
            </div>
            <div>
              <p class="text-xs text-primary/45 uppercase">
                Waktu Baca</p>
              <p class="font-medium text-primary mt-0.5">3
                Menit</p>
            </div>
            <div>
              <p class="text-xs text-primary/45 uppercase">
            ...
            <div>
              <p class="text-xs text-primary/45 uppercase">
                Tanggal Rilis</p>
              <p class="font-medium text-primary mt-0.5">
                {{ \Carbon\Carbon::parse($article->published_date)->locale('id')->translatedFormat('d F Y') }}
              </p>
            </div>
          </div>
        </div>
      ...
      <div class="space-y-6">
        <div>
          <span
            class="text-accent text-xs font-semibold uppercase tracking-wider">{{ $article->category }}</span>
          <h1
        ...
        <div
          class="border-l-4 border-accent pl-4 py-1.5 italic text-primary/70 text-base md:text-lg leading-relaxed bg-accent/5 rounded-r-lg">
          {{ $article->excerpt }}
        </div>

        <!-- Main Article Content paragraphs -->
        <div
          class="text-sm md:text-base text-primary/80 leading-relaxed space-y-6">
          @if ($article->content)
        ...
        <div
          ...
          <button type="button"
            ...
            <svg class="w-4 h-4" fill="none"
              ...
              <path stroke-linecap="round"
                stroke-linejoin="round"
                d="M8.684 10.742l4.828-2.414m0 0a3 3 0 10-3.62-1.09l-4.829 2.414m0 0a3 3 0 100 4.284l4.829 2.414m0 0a3 3 0 103.62-1.09" />
            </svg>
            Bagikan
          </button>
        </div>
    ...
    @if (!$relatedArticles->isEmpty())
      <div class="mt-24 border-t border-primary/10 pt-16">
        <div
          class="flex flex-col sm:flex-row sm:items-end justify-between gap-3 sm:gap-4 mb-8 md:mb-12">
          <div>
            <h2 class="text-2xl font-semibold text-primary">
              Artikel Terkait</h2>
            <p class="text-xs text-primary/50 mt-1">Tulisan
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
          @foreach ($relatedArticles as $related)
            <div
              class="group bg-white rounded-xl overflow-hidden shadow-sm hover:shadow-md transition-shadow duration-300 flex flex-col h-full border border-primary/5">
              <a href="/article/{{ $related->slug }}"
                wire:navigate class="block">
                <div
                  class="overflow-hidden bg-primary/[0.02]">
              ...
              <div class="p-5 flex flex-col flex-1">
                <a href="/article/{{ $related->slug }}"
                  ...
                  <span
                    ...
                    {{ $related->category }} •
                    {{ \Carbon\Carbon::parse($related->published_date)->locale('id')->translatedFormat('d F Y') }}
                  </span>
                  <h3
                    class="font-semibold text-primary text-base leading-snug mt-2 hover:text-accent transition-colors line-clamp-2">
                    {{ $related->title }}</h3>
                </a>
              </div>
            </div>
