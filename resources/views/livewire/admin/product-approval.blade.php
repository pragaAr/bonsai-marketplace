<div class="space-y-6">
  <!-- Header -->
  <div>
    <h1 class="text-2xl font-bold text-primary">Persetujuan
      Produk</h1>
    <p class="text-sm text-primary/60 mt-1">Tinjau dan kelola
      kelayakan produk yang diajukan oleh penjual</p>
  </div>

  <!-- Search & Filter Card -->
  <div
    class="bg-white rounded-xl shadow-sm border border-primary/10 p-5">
    <div
      class="flex flex-col md:flex-row gap-4 items-center justify-between">
      <div class="w-full md:w-1/2 relative">
        <span
          class="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none text-primary/40">
      ...
      <div
        class="w-full md:w-auto flex flex-wrap gap-3 items-center">
        <div class="relative w-full md:w-48">
          ...
          <div
            class="pointer-events-none absolute inset-y-0 right-0 flex items-center px-2 text-primary/50">
  ...
  <!-- Table Card -->
  <div
    class="bg-white rounded-xl shadow-sm border border-primary/10 overflow-hidden">
    <div class="overflow-x-auto">
      <table
        class="w-full text-left border-collapse text-sm">
        <thead>
          <tr
            class="bg-cream/20 border-b border-primary/10 text-primary/60 font-semibold">
        ...
        <tbody class="divide-y divide-primary/5">
          @forelse($products as $prod)
            <tr
              class="hover:bg-cream/5 transition duration-150">
              <!-- Thumbnail & Name -->
              <td class="px-6 py-4">
              ...
              <!-- Price -->
              <td
                class="px-6 py-4 font-semibold text-primary">
                Rp
          ...
          @empty
            <tr>
              <td colspan="6"
                class="px-6 py-12 text-center text-primary/50">
                ...
                Antrean produk kosong. Tidak ada produk yang
                perlu ditinjau.
              </td>
            </tr>
          @endforelse
  ...
  @if ($showDetailModal && $selectedProduct)
    <div
      ...
      <div
        class="fixed inset-0 bg-primary/50 backdrop-blur-sm transition-opacity">
      </div>
      <div
        class="relative w-full max-w-4xl mx-auto my-6 px-4 z-50">
        <div
          class="bg-white rounded-xl shadow-2xl border border-primary/10 overflow-hidden max-h-[90vh] flex flex-col">
          <!-- Modal Header -->
          <div
            class="flex items-center justify-between p-5 border-b border-primary/10 bg-cream/10 flex-shrink-0">
            <div>
              <span
                class="text-accent text-[10px] font-bold uppercase tracking-wider bg-accent/5 px-2 py-0.5 rounded">
                {{ $selectedProduct->category->name }}
              </span>
              <h3
                class="text-lg font-bold text-primary mt-1">
                Review: {{ $selectedProduct->name }}</h3>
            </div>
            <button
              wire:click="$set('showDetailModal', false)"
              class="text-primary/60 hover:text-primary transition duration-150">
              <svg xmlns="http://www.w3.org/2000/svg"
          ...
          <div
            class="p-6 overflow-y-auto space-y-6 flex-1 text-sm text-primary">
            ...
            <div
              ...
              <div class="space-y-3">
                <p
                  class="text-xs font-semibold text-primary/50 uppercase tracking-wider">
                  Gambar Produk</p>
                <div class="grid grid-cols-2 gap-3">
                  @forelse($selectedProduct->getMedia('images') as $media)
                    <a href="{{ $media->getUrl() }}"
                      target="_blank"
                      class="block aspect-square rounded-lg border border-primary/10 overflow-hidden bg-cream hover:opacity-90 transition duration-150">
                  ...
                  @empty
                    ...
                    <div
                      class="col-span-2 aspect-square rounded-lg border border-dashed border-primary/20 flex flex-col items-center justify-center text-primary/40">
                      <svg
                        xmlns="http://www.w3.org/2000/svg"
                        class="h-8 w-8 mb-2"
                        fill="none" viewBox="0 0 24 24"
              ...
              <!-- Basic details -->
              <div
                class="space-y-4 bg-cream/10 border border-primary/5 rounded-xl p-5">
                <p
                  class="text-xs font-semibold text-primary/50 uppercase tracking-wider border-b border-primary/5 pb-1">
                  Detail Listing</p>
                <div
                  ...
                  <div>
                    <span
                      ...
                      Pada</span>
                    <span
                      class="text-xs text-primary/80 block">{{ $selectedProduct->created_at->format('d M Y, H:i') }}</span>
                  </div>
                </div>
            ...
            <div class="space-y-2">
              <p
                ...
                Deskripsi Produk</p>
              <div
                class="bg-cream/10 border border-primary/5 rounded-xl p-5 leading-relaxed">
                <p
                  class="font-medium text-primary text-sm">
                  {{ $selectedProduct->short_description }}
                </p>
                <div class="h-px bg-primary/5 my-2.5">
                </div>
                <p
            ...
            <div class="space-y-2">
              <p
                class="text-xs font-semibold text-primary/50 uppercase tracking-wider">
                Spesifikasi Tambahan (Polymorphic)</p>
              <div
                class="bg-white border border-primary/10 rounded-xl p-5 grid grid-cols-2 sm:grid-cols-3 gap-4">
            ...
            @if ($selectedProduct->status === 'rejected')
              <div
                class="bg-red-50 border border-red-200 rounded-xl p-5 space-y-1">
                <span
                  class="text-xs text-red-600 block uppercase font-bold tracking-wider">Alasan
                  Penolakan Sebelumnya</span>
            ...
            @elseif($selectedProduct->status === 'approved')
              <div
                class="bg-green-50 border border-green-200 rounded-xl p-5 flex items-center justify-between">
                <div>
                  <span
                  ...
                  <p class="text-green-900 font-medium">
                    Disetujui oleh:
                    {{ optional($selectedProduct->approvedBy)->name }}
                  </p>
                </div>
                <div class="text-right">
                  <span
                    ...
                    Approval</span>
                  <span
                    class="text-xs text-green-950 font-bold block">{{ $selectedProduct->approved_at ? $selectedProduct->approved_at->format('d M Y, H:i') : '-' }}</span>
                </div>
              </div>
            @endif
            ...
            @if ($isRejecting)
              <div
                class="bg-cream/20 border border-primary/20 rounded-xl p-5 space-y-3">
                <label for="rejectionReason"
                  class="block text-xs font-bold text-red-600 uppercase">Mengapa
                  Produk Ini Ditolak? <span
          ...
          <div
            class="p-5 border-t border-primary/10 bg-cream/10 flex items-center justify-between flex-shrink-0">
            <div>
              <button
                wire:click="$set('showDetailModal', false)"
