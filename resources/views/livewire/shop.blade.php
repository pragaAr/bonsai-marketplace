<div>
  <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-24">
    ...
    <div class="mb-8 pb-4 border-b border-primary/10">
      <h1 class="text-3xl font-semibold text-primary">Koleksi Kami</h1>
      <p class="text-sm text-primary/50 mt-1">Bermacam produk dari komunitas untuk komunitas</p>
    </div>
    ...
    <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-4 mb-10">
      ...
      <div x-data="{ showAll: false }" class="flex gap-2 overflow-x-auto pb-2 md:pb-0 scrollbar-hide whitespace-nowrap md:overflow-x-hidden" :class="showAll ? 'md:!overflow-x-auto' : ''">
    ...
    @if(!empty(trim($search)))
      <div class="mb-6 flex items-center gap-2">
        <span class="text-xs bg-primary/5 text-primary/60 px-3 py-1 rounded-full flex items-center gap-1.5">
          Search: "{{ $search }}"
          <button wire:click="$set('search', '')" class="hover:text-red-500 font-bold">×</button>
        </span>
      </div>
    @endif
    ...
    <!-- Product Grid -->
    @if($products->isEmpty())
      <div class="text-center py-20 bg-white rounded-xl border border-primary/5 shadow-sm">
        <svg class="w-16 h-16 text-primary/10 mx-auto mb-4" fill="none" stroke="currentColor" stroke-width="1.5" viewBox="0 0 24 24">
          <path stroke-linecap="round" stroke-linejoin="round" d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3z"/>
        </svg>
        <h3 class="text-base font-semibold text-primary">No Specimens Found</h3>
        <p class="text-xs text-primary/50 mt-1 max-w-xs mx-auto">We couldn't find any bonsai trees matching your current criteria. Try adjusting your filters or search term.</p>
        <button wire:click="$set('category', 'All'); $set('search', ''); $set('sort', 'default')" class="mt-4 inline-flex text-xs text-accent hover:underline">
          Reset Filters
        </button>
      </div>
    @else
      <div class="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-4 gap-4 md:gap-6 lg:gap-8 stagger-children">
        @foreach($products as $product)
          <div class="group bg-white rounded-xl overflow-hidden shadow-sm hover:shadow-md transition-shadow duration-300 flex flex-col h-full" wire:key="product-{{ $product->id }}">
            ...
            <div class="p-4 flex flex-col flex-1">
              ...
              <!-- Cart & Buy Actions -->
              <div class="flex flex-wrap flex-shrink gap-2 mt-4 pt-3 border-t border-primary/5">
