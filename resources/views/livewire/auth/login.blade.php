    ...
    <div class="glass rounded p-8 shadow-xl">
      ...
      <form wire:submit="login" class="space-y-5" novalidate>
        ...
        <div class="mb-4">
          ...
          <input wire:model.defer="email" type="email" name="email" id="email" required autofocus class="w-full border border-primary/15 rounded px-3 py-2 focus:outline-none focus:border-primary/40" placeholder="Masukan email anda" autocomplete="email" autofocus/>
          @error('email')
            <small class="text-xs text-red-600 mt-1">{{ $message }}</small>
          @enderror
        </div>
        ...
        <div x-data="{ showPassword: false }" class="mb-4">
          ...
          <div>
            <div class="relative">
              <input :type="showPassword ? 'text' : 'password'" wire:model.defer="password" name="password" id="password" required class="w-full border border-primary/15 rounded px-3 py-2 pr-10 focus:outline-none focus:border-primary/40" placeholder="••••••••" autocomplete="current-password" />
              <button type="button" @click="showPassword = !showPassword" class="text-secondary-400 hover:text-secondary-600 absolute inset-y-0 right-0 flex cursor-pointer items-center justify-center px-4 focus:outline-none">
                <svg x-show="!showPassword" class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke-width="1" stroke="currentColor">
                  <path stroke-linecap="round" stroke-linejoin="round"
                    d="M2.036 12.322a1.012 1.012 0 010-.639C3.423 7.51 7.36 4.5 12 4.5c4.638 0 8.573 3.007 9.963 7.178.07.207.07.431 0 .639C20.577 16.49 16.64 19.5 12 19.5c-4.638 0-8.573-3.007-9.963-7.178z" />
                  <path stroke-linecap="round" stroke-linejoin="round" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z" />
                </svg>
                <svg x-show="showPassword" style="display: none;" class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke-width="1" stroke="currentColor">
                  <path stroke-linecap="round" stroke-linejoin="round"
            ...
            @error('password')
              <small class="mt-1 block text-xs text-red-600">{{ $message }}</small>
            @enderror
          </div>         
        </div>
        <div class="flex items-center justify-between mb-4">
          <label class="inline-flex items-center">
            <input wire:model.defer="remember" type="checkbox" name="remember" class="form-checkbox text-primary" />
