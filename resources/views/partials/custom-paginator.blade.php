  ...
  <nav role="navigation" aria-label="Pagination Navigation"
    ...
    <div class="flex justify-between flex-1 sm:hidden">
      @if ($paginator->onFirstPage())
        <span
          class="relative inline-flex items-center px-3 py-1.5 text-xs font-medium text-gray-400 bg-white border border-gray-300 rounded-md cursor-default select-none">
          Previous
        </span>
      @else
        <button type="button" wire:click="previousPage"
          wire:loading.attr="disabled"
          class="relative inline-flex items-center px-3 py-1.5 text-xs font-medium text-[#2d3e2fec] bg-white border border-gray-300 rounded-md cursor-pointer hover:bg-gray-50 focus:outline-none">
          Previous
        </button>
      @endif
      ...
      @if ($paginator->hasMorePages())
        <button type="button" wire:click="nextPage"
          wire:loading.attr="disabled"
          class="relative inline-flex items-center px-3 py-1.5 ml-3 text-xs font-medium text-[#2d3e2fec] bg-white border border-gray-300 rounded-md cursor-pointer hover:bg-gray-50 focus:outline-none">
          Next
        </button>
      @else
        <span
          class="relative inline-flex items-center px-3 py-1.5 ml-3 text-xs font-medium text-gray-400 bg-white border border-gray-300 rounded-md cursor-default select-none">
          Next
        </span>
      @endif
    </div>
        ...
        <span
          ...
          @foreach ($elements as $element)
            ...
            @if (is_string($element))
              <span aria-disabled="true"
                class="relative inline-flex items-center px-3 py-1.5 -ml-px text-xs font-medium text-gray-400 bg-white border border-gray-300 cursor-default select-none">{{ $element }}</span>
            @endif
            ...
            @if (is_array($element))
              @foreach ($element as $page => $url)
                @if ($page == $paginator->currentPage())
                  {{-- Menggunakan style inline style="background-color: #2d3e2fec" sebagai jaminan warna teks putih selalu terlihat jelas --}}
